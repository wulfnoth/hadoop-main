...
public abstract class FileSystemLinkResolver<T> {
	...
	private final static WBlank blank = WBlankFactory.getWBlank(FileSystemLinkResolver.class);
	 ...
	 */
	abstract public T doCall(final Path p) throws
	 ...
	 * 尝试根据指定的FileSystem和Path调用重载的doCall方法。
	 * 如果调用时抛出了UnresolvedLinkException，将会尝试解析path，并且通过调用next(FileSystem, Path)
	 * 方法进行重试。
	...
	public T resolve(final FileSystem filesys, final Path path)
		...
		for (boolean isLink = true; isLink;) {
			try {
				...
				blank.info("Symlink Resolution is enable? : " + filesys.resolveSymlinks);
				blank.info("File System is supported? : " + FileSystem.areSymlinksEnabled());
